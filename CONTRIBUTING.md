...
Two files pin exact versions:

| File | Holds | Installed by |
|---|---|---|
| `requirements.lock` | the core: everything the audit engine needs | everyone |
| `requirements-pathology.lock` | what the `[pathology]` extra adds on top | Track P users, and the automated checks |

**The rule: a lock file is regenerated only in a clean environment, never in
the one you work in.** A working environment accumulates tools that are not
dependencies — matplotlib for figures, a debugger, something you tried once —
and `pip freeze` would capture all of them as if the project needed them.

The procedure (the same one written at the top of `requirements-pathology.lock`):

```bash
python -m venv /tmp/lockenv && source /tmp/lockenv/bin/activate   # Windows: py -3.13 -m venv ...; .\lockenv\Scripts\Activate.ps1
python -m pip install --upgrade pip

# core
python -m pip install -e . --no-deps && python -m pip install .   # resolves pyproject's ranges fresh
python -m pip freeze --exclude-editable | sort > requirements.lock.new

# extra: what it adds beyond the core
python -m pip freeze --exclude-editable | sort > /tmp/core.txt
python -m pip install ".[pathology]"
python -m pip freeze --exclude-editable | sort > /tmp/all.txt
comm -13 /tmp/core.txt /tmp/all.txt > requirements-pathology.lock.new
comm -23 /tmp/core.txt /tmp/all.txt          # MUST print nothing: the extra may not change a core pin
```

Then diff `.new` against the current files, keep the header comments, and
commit both together. `tests/test_python_support.py` checks that the packages
which set the Python floor are still pinned; if regenerating raises the floor,
`pyproject.toml`'s `requires-python` and the setup guides change in the same
commit.

## Figures are code

Every image under `docs/img/` is generated by `scripts/make_figures.py`, from
the project's own data, with fixed seeds. If a figure needs changing, change
the script and regenerate — never edit an image by hand, and never add a
hand-drawn one. This keeps the documentation to the same standard as the
results: anything a reader sees can be rebuilt from the repository.

The script needs matplotlib, which is deliberately not a project dependency
(the audit engine has no business carrying a plotting stack). Install it ad
hoc when working on figures:
...
Diagrams drawn in text (the boxes-and-arrows in the guides) use Mermaid, which
GitHub renders natively — they are part of the Markdown itself and need no
tooling at all.
