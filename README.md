...
![tests](https://img.shields.io/badge/tests-74%20passing-brightgreen?logo=pytest&logoColor=white)
...
| Status | foundation released (v0.1.1) | designed and documented; first code landed — the synthetic H&E phantom with 16 checks |
...
| **Medical Segmentation Decathlon, Task 04 (Hippocampus)** · [card](docs/data-cards/msd-hippocampus.md) | 390 real T1 MRI volumes, 260 with expert outlines (two parts), about 27 MB, CC-BY-SA 4.0, no account needed. Fetched by checksum-verified download; tiny volumes, so everything runs on a CPU. | ✅ phase V2 |
...
| **Synthetic IHC and mIF phantoms** (Track P, built in) | A synthetic IHC channel with known positive fraction; a multichannel mIF phantom with known phenotype proportions. | ⬜ phase P1 |
...
- **Phase 1 — Skeleton:** ✅ an installable package with a strict layering
  ...
  a 4-command CLI, 74 tests that run in under a second with no download, and
...
- **Phase V2 — Real data (Track V):** ✅ a dataset registry with
  ...
  patient anywhere. 20 checks, none touching the network.
...
stableseg fetch msd_task04_hippocampus      # real MRI, 27 MB, checksum-verified
