...
![real data](https://img.shields.io/badge/real%20data-MSD%20hippocampus%2C%20checksum%20verified-2E7D32)
![tests](https://img.shields.io/badge/tests-92%20passing-brightgreen?logo=pytest&logoColor=white)
...
![data](https://img.shields.io/badge/data-6%20data%20cards%20%2B%20MRI%20%26%20H%26E%20phantoms-6f42c1)
...
| Status | foundation released (v0.1.1); real MRI fetched and catalogued | H&E, IHC and mIF phantoms; OME-TIFF and whole-slide reading; the `[pathology]` extra |
...
| **Medical Segmentation Decathlon, Task 04 (Hippocampus)** · [card](docs/data-cards/msd-hippocampus.md) | 390 real T1 MRI volumes, 260 with expert outlines (two parts), 28 MB, CC-BY-SA 4.0, no account needed. Fetched by checksum-verified download; tiny volumes, so everything runs on a CPU. | ✅ phase V2 |
| **Synthetic H&E phantoms** (Track P, built in) | 256 px tiles at 0.5 µm/px; 60 nuclei as ellipses coloured through the hematoxylin/eosin optical-density model; known counts, positions, sizes; fixed seed. `src/stableseg/pathology/phantom.py`. **Synthetic, labelled as such everywhere.** | ✅ phase P1a |
| **Synthetic IHC and mIF phantoms** (Track P, built in) | IHC: 60 nuclei with a set 35% stained brown through the DAB optical-density vector — the positive fraction is the truth. mIF: five named channels (DAPI, PanCK, CD3, CD8, CD68) as OME-TIFF; every cell's phenotype known. **Synthetic, labelled as such.** | ✅ phase P1 |
...
- **Phase 1 — Skeleton:** ✅ an installable package with a strict layering
  ...
  a 4-command CLI, 92 tests that run in under a second with no download, and
...
- **Phase P1a — Synthetic H&E phantom (Track P):** ✅ a 2-D `Tile`
  abstraction that never separates a picture from its microns-per-pixel; a
  seeded generator drawing nuclei as ellipses and colouring them through the
  hematoxylin/eosin optical-density model; 16 checks; the reference value
  **2950.875 µm²** (mean nuclear area per tile) reproduces on every machine.
...
- **Phase V2 — Real data (Track V):** ✅ a dataset registry with
  checksum-verified, idempotent fetching (a mismatched archive is deleted,
  never used); a catalogue that reads every scan's voxel spacing from its
  header; and a DICOM reader converting the hospital coordinate frame to the
  research one, tested against a series generated with pydicom and read back
  with SimpleITK — two implementations of the standard agreeing, with no
  patient anywhere. 20 checks, none touching the network. First observed
  biomarker values: `hippocampus_001` anterior 1,324 mm³, posterior 1,624 mm³.

  ![A real hippocampus MRI in three views with the expert outline](docs/img/msd_hippocampus_case.png)

- **Phase P1 — Pathology data and I/O (Track P):** ✅ one geometry contract
  that `Volume` and `Tile` both inherit; IHC and mIF phantoms whose truths
  (positive fraction 0.35; phenotype counts 36/12/12/8/12) reproduce exactly;
  OME-TIFF with channel names inside the file; a pyramidal-TIFF writer and an
  OpenSlide reader that streams tissue tiles from a whole slide, tested on a
  slide built from the phantom. The `[pathology]` extra with a clean-room
  lock file, installed by CI on all six platform combinations. 18 checks.

  ![IHC and mIF phantoms with known truths](docs/img/pathology_phantoms.png)

  ![Synthetic slide, tissue mask, selected tiles, one streamed tile](docs/img/synthetic_slide.png)

- **Phase V3 — MRI perturbation bank:** *(pending)*.
...
| P1a | [Synthetic H&E phantom — Track P's first code](docs/04-phase-tutorials/phase-P1a-he-phantom.md) | ✅ |
| V2 | [Real data: verified fetch, catalogue, DICOM reader](docs/04-phase-tutorials/phase-V2-real-data.md) | ✅ |
| P1 | [Pathology data & I/O: geometry base, IHC + mIF phantoms, OME-TIFF, OpenSlide reader, pathology extra](docs/04-phase-tutorials/phase-P1-pathology-data.md) | ✅ |
| P1b | Real slide streamed, spatial-transcriptomics reader, pathology dataset registry | ⬜ next |
...
├── requirements-pathology.lock    ← exact pins for the [pathology] extra, clean-room resolved
├── configs/
│   ├── phantom.yaml               ← a complete run config: generate MRI phantoms
│   ├── msd_hippocampus.yaml       ← points later phases at the fetched real data
│   └── he_phantom.yaml            ← a complete run config: generate H&E phantoms (Track P)
...
│   ├── test_he_phantom.py         ← Track P: tile geometry, OD model, known truth, determinism
│   ├── test_datasets_and_dicom.py ← fetch via file://, checksums, unpack safety, DICOM round trip
│   └── test_pathology_io.py       ← geometry contract, OME-TIFF, pyramid → OpenSlide, IHC/mIF truths
...
python -m pip install -r requirements-pathology.lock   # optional: whole-slide reading (Track P)
pytest -q                                   # expect: 92 passed (87 without the extra, 5 skipped)
stableseg phantom                           # Track V: data/phantom/, checkpoint 2269.75
stableseg describe data/phantom/images/phantom_000.nii.gz
stableseg he-phantom                        # Track P: data/he_phantom/, checkpoint 2950.875
stableseg describe-tile data/he_phantom/images/he_phantom_000.png
stableseg ihc-phantom                       # Track P: positive fraction 0.35
stableseg mif-phantom                       # Track P: five channels, phenotypes known
stableseg fetch msd_task04_hippocampus      # real MRI, 28 MB, checksum-verified
