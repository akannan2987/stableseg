...
def fig_he_phantom() -> None:
    ...
    axes[0].imshow(rgb)
    axes[0].set_title("synthetic H&E tile\n(256 px · 0.5 µm/px · 128 µm field)", fontsize=10)

    axes[1].imshow(labels > 0, cmap="gray")
    axes[1].set_title(f"known truth: {len(truths)} nuclei\n(instance label map)", fontsize=10)

    axes[2].imshow(h_channel, cmap="magma")
    axes[2].set_title("hematoxylin channel recovered by\ncolour deconvolution (preview of P3)", fontsize=10)

    axes[3].imshow(rgb)
    axes[3].contour(labels > 0, levels=[0.5], colors=[GREEN], linewidths=0.9)
    axes[3].set_title("truth outlines over the picture", fontsize=10)
    for ax in axes:
        ax.axis("off")
    fig.suptitle(
    ...
    fig.tight_layout(rect=(0, 0, 1, 0.90))
    fig.savefig(IMG / "he_phantom_tile000.png", dpi=150)
    plt.close(fig)
...
if __name__ == "__main__":
    ...
    for f in sorted(IMG.glob("*.png")):
        ...
        print(f"  {f.relative_to(REPO_ROOT)}  {kb:.0f} KB")
