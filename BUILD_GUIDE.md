...
**Status: built.** The first real scans, and the first figure in the project
that is not synthetic.

### What it is

A **registry** of datasets — URL, checksum, licence, data card — and a fetch
command that downloads to a temporary name, verifies the fingerprint, refuses
and deletes on mismatch, unpacks while dropping the hidden `._` companions Mac
archives carry, and does nothing the second time it is run. A **catalogue**
that reads every scan's header into one table so voxel spacing is known, not
assumed. And a **DICOM reader** — the hospital format, one file per slice —
that converts the hospital coordinate convention to the research one so every
scan lands in one frame, tested against a series generated by code so no
patient data exists anywhere.

### Why now, and why this dataset

You cannot write a realistic disturbance without a real image to disturb.
This dataset because the volumes are tiny — about 35 × 50 × 35 voxels — so
everything runs on an ordinary processor in minutes, and because hippocampal
volume is a *real* trial endpoint, so the question the project asks is real
rather than invented. Everything about it is on its
[data card](docs/data-cards/msd-hippocampus.md).

### What you will understand afterwards

What a checksum is and why a download is verified rather than trusted. Why an
archive can lie about its own contents. How real scans differ from generated
ones. The difference between the research format and the hospital format,
why they disagree about which way is left, and how to test a reader for a
format you have no legal data for.

### 🔗 The detail

[`docs/04-phase-tutorials/phase-V2-real-data.md`](docs/04-phase-tutorials/phase-V2-real-data.md)
— the fetch explained from zero, the catalogue, a real scan described, DICOM
from one file per slice to a volume, and the checks.
...
*The first real figure: one subject's T1 MRI in sagittal, coronal and axial
views, with the expert outline of the two hippocampal parts. Generated on
your machine after the fetch — the data itself never enters the repository.*

### Commit it

```bash
ls docs/img/msd_hippocampus_case.png         # the figure travels with the phase
