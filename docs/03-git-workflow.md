...
**Checkpoint:** `git branch -vv` shows `develop` and `master` on the same commit, each tracking its remote; GitHub shows `master` as the default branch and `beta` as a third branch; one commit has arrived on all three.
