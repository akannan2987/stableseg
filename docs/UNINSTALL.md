...
```mermaid
flowchart TB
    L1["🥕 data/ and runs/<br/>regenerated by code — delete freely"] --> L2["🧰 .venv<br/>this project's toolbox — disposable by design"]
    L2 --> L3["📁 the project folder<br/>safe once everything is pushed"]
    L3 --> L4["🔥 Python · Git · VS Code · R<br/>shared with other work — think first"]

    classDef safe fill:#E6F4EA,stroke:#4CAF7D,color:#0B3D2E;
    classDef mid fill:#FFF3CD,stroke:#C9A227,color:#4A3B00;
    classDef care fill:#FDE8E8,stroke:#CC3311,color:#5A1010;
    class L1,L2 safe
    class L3 mid
    class L4 care
```

*Read top to bottom: each layer is safer to remove than the one below it, and
the sections of this page follow the same order.*
