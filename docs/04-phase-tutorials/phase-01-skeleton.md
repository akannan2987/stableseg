...
**Checkpoint:** `pytest -q` prints `54 passed`; `stableseg phantom` writes eight phantom files and a `run.json`; the CI badge on GitHub turns green on all three operating systems.
...
pytest -q
```
Expected: `54 passed in 0.5s` (time varies). `-q` is quiet; drop it to see each test's name. `pytest -q --cov=stableseg --cov-report=term-missing` adds a coverage table: which lines the tests exercised.
