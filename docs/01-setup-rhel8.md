...
With the environment active, in the project folder — **upgrade pip first**,
because RHEL 8's stock pip is too old to read the wheel's platform tag:
...
`openslide-bin` wheel, built for glibc 2.28 or newer — which RHEL 8 has — so
no `dnf install openslide` and no EPEL package is needed.
...
| Symptom | Cause | Fix |
|---|---|---|
| `No matching distribution found for openslide-bin` | pip too old to recognise the `manylinux_2_28` wheel | `python -m pip install --upgrade pip`, retry |
| `GLIBC_2.28 not found` | a system older than RHEL 8 | the extra needs glibc 2.28+; the core still works |
